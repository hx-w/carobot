...
class Check():
    ...
    async def get_check_info(self):
        ...
        putline.append("--Performance--\n[Cpu] {}%\n[Memory] {}%\n[Disk] {}%\n--Network--\n[Baidu] {}\n[CSGO Wiki] {}".format(self.cpu_percent, self.memory_percent, self.disk_percent, self.baidu, self.csgowiki))
    ...
    async def get_check_easy(self, max_performance_percent=[92,92,92]):
        putline = []
        check_list = await self.get_check_simple(max_performance_percent)
    ...
    def get_network_check(self):
        ...
        try:
            self.csgowiki = requests.get("https://www.csgowiki.top").status_code
        except:
            self.csgowiki = 404
